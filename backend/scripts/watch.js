...
async function watch(options = DEFAULTS) {
  ...
  // Creates the log & pid directory if it doesn't exist
  await fsPromises.mkdir(path.dirname(options.log), { recursive: true });
  await fsPromises.mkdir(path.dirname(options.pid), { recursive: true });

  // Logger is used to log everything else than the server output
  const logger = await createLogger(options);
  logger.write('Inner process logs ready\n');
...
async function runInitHooks(options, logger) {
  ...
  for (const hook of options.hooks) {
    ...
    logger.write(`Running init hook ${hook.exec}...\n`);
...
async function createProcess(cmd, args, options, logger, cwd = options.cwd ?? process.cwd()) {
  const cp = childProcess.spawn(formatBin(cmd), args, {
    cwd: cwd,
    env: process.env,
    stdio: ['ignore', logger, logger]
  });
 ...
 */
function createShutdownHook(cp) {
  ...
  async function stopHook() {
    ...
    if (cp.exitCode || cp.killed || onShutdown.closing) {
  ...
  // Creates a shutdown hook for this process
  const hook = onShutdown(function shutdownHook() {
    // Removes the hook so it doesn't run again
